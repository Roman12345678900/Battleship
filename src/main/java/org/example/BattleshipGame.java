...
class BattleshipGame {
    ...
    public static void drawDualField(int[][] playerField, int[][] opponentField) {
        String separator = " | ";

        // Рисуем поле текущего игрока (с кораблями)
        System.out.print("  ");
        ...
        // Выводим разделитель между полями
        System.out.print(separator);

        // Рисуем поле атак на поле противника:
        System.out.print("  ");
        ...
        for (int i = 0; i < playerField.length; i++) {
            ...
            for (int j = 0; j < playerField[1].length; j++) {
                if (playerField[j][i] == 0) {
                    System.out.print(" -");
                } else if (playerField[j][i] == 1) {
                    System.out.print(" *"); // Корабль текущего игрока
                } else {
                    System.out.print(" ."); // Промах по текущему игроку
                }
            }
            ...
            System.out.printf("%2d", i + 1);
            for (int j = 0; j < opponentField[1].length; j++) {
                if (opponentField[j][i] == 0) {
                    System.out.print(" -");
                } else if (opponentField[j][i] == 2) {
                    System.out.print(" X"); // Попадание по противнику
                } else {
                    System.out.print(" ."); // Промах по противнику
                }
            }
            System.out.println();
        }
